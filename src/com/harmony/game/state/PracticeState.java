...
public class PracticeState extends State {
    ...
    public void onCreate() {
        ...
        console = new Console();
        player = new Player(null, (ObjectTileMap) tileManager.getObjectsMap());
        console.setShowConsole(true);

        console.pushMessage("Welcome to Crystals!!! Use W A S D to move around the screen...");
    }
    ...
    public void update() {
        ...
        if((player.dx != 0 || player.dy != 0) && move) {
            move = false;
            loadTimer = true;
            console.pushMessage("Great Job!!!");
            stopTime = System.currentTimeMillis() + delay;
        }

        if(loadTimer) {
            if(System.currentTimeMillis() >= stopTime) {
                console.pushMessage("Explore the map on your own... Try to find and open the chest...");
                loadTimer = false;
        ...
        if(!hasKey && GUI.hasKey) {
            hasKey = true;
            console.setShowConsole(true);
            console.pushMessage("Great Job... Notice the key collected in the upper right hand corner.\n\n" +
